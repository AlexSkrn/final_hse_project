...
Программа сравнивает перевод, введенный пользователем, с переводом-ориентиром,
содержащимся в программе, с помощью коэффициента __BLEU__. Этот коэффициент
рассчитывается функцией из библиотеки NLTK. Коэффициент учитывает порядок слов
(на основе совпадения биграмм) и длину перевода. Форма слова на него не влияет,
поскольку в функцию подаются уже лемматизированные предложения (с помощью __pymystem3__).
